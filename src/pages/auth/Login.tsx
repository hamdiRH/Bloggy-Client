...
export function Login() {
  ...
  async function loginDev(event: any) {
    event.preventDefault();

    const emailDev = import.meta.env.VITE_ADMIN_EMAIL
    const passwordDev = import.meta.env.VITE_ADMIN_PASSWORD
    try {
      const { data } = await auth.login(emailDev, passwordDev);
      localStorage.setItem("token", data.token);
